...
-- devops_ci_repository.T_REPOSITORY_SESSION_INFO definition

CREATE TABLE IF NOT EXISTS `T_REPOSITORY_SESSION_INFO` (
  `ID` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `SCM_TYPE` varchar(64) NOT NULL COMMENT '代码库类型',
  `AUTH_USER_NAME` varchar(100) NOT NULL COMMENT '代码库登录态用户名',
  `PRIVATE_TOKEN` varchar(256) DEFAULT NULL COMMENT '私人令牌',
  `CREATE_TIME` datetime DEFAULT NULL COMMENT '创建时间',
  `UPDATE_TIME` datetime DEFAULT NULL COMMENT '更新时间',
  PRIMARY KEY (`ID`),
  INDEX `IDX_REPOSITORY_SESSION_INFO` (`SCM_TYPE`, `AUTH_USER_NAME`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='代码库登录态信息';
